...
- Keep publication handoff capability-safe: clipboard, Web Share, Telegram, and WhatsApp actions must receive only the public recipient URL. Never place the management URL or token in a share payload; capability-detect native sharing and always keep manual copy available as the fallback.
