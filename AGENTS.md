...
- Route every successful publication to one dedicated author-only result page. Present the public recipient link as the primary share action, keep the management recovery link hidden behind an explicit reveal, never combine both in one copy action, and keep management tokens in URL fragments only long enough to move them into session storage.
