...
- Keep the real recipient decline action playful but bounded: for pointer input the button may move on the first four attempts, while the fifth attempt must stop moving and submit the decline. A failed save must leave an explicit retry available. Keyboard activation and `prefers-reduced-motion` must keep decline directly accessible. A persisted decline must render a separate neutral stage, allow one explicit change to acceptance only before final confirmation, and never expose a decline control after a confirmed plan exists.
