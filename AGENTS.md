...
- Treat the final message as a data-only template. Allow only `{author}`, `{recipient}`, `{date}`, `{time}`, `{place}`, and `{activity}`; reject traversal, indexing, conversions, format specifications, malformed braces, and unknown variables on both backend and frontend. Render values as plain text and keep non-final screen templates empty.
