...
Дата должна быть в будущем и содержать часовой пояс, место обязательно, а комментарий варианта можно оставить пустым. Пока получатель ничего не выбрал, повторный `PUT` атомарно заменяет набор вариантов. После выбора набор блокируется от изменений.
...
Повтор того же выбора идемпотентен, а до будущего этапа итогового подтверждения получатель может выбрать другой вариант.
...
| `DRF_THROTTLE_PLAN_RATE` | Django REST Framework | Дополнительный лимит изменения вариантов и выбора, по умолчанию `20/min` |
...
- API создания, публичного чтения, сохранения ответа, планирования и защищённого чтения автора через `/api/v1/`.
...
- Общий и отдельные для создания и ответа rate limits с ответом `429 Too Many Requests`.
...
Следующий этап — итоговое подтверждение выбранного варианта автором и финальный экран плана для обеих сторон. После него можно добавить календарные файлы, напоминания и аккуратную возможность вернуться к согласованию.
