...
Date Planner — веб-сервис для создания персональных приглашений на свидание и последующего совместного планирования. Сейчас проект предоставляет форму создания приглашения, персональную интерактивную страницу для получателя, защищённую ссылку автора и основу monorepo с запуском через Docker.
...
| Публичное приглашение | `http://localhost:3000/invite/<uuid>` |
...
Получить публичные данные одного приглашения можно по адресу:
...
Frontend создаёт две ссылки:

- публичную `/invite/<uuid>` — её отправляют получателю;
- секретную `/manage/<uuid>#token=<management-token>` — её сохраняет автор.
...
Каждое приглашение хранится отдельной записью, поэтому данные разных людей не перезаписывают друг друга. Публичного `GET /api/v1/invitations/` нет: API не раскрывает общий список приглашений. Существующие до миграции записи без management-токена остаются доступными только по публичной ссылке; при необходимости управления их следует создать заново.

Все API-запросы ограничены по частоте. По умолчанию анонимному клиенту разрешено `60` запросов в минуту, будущему авторизованному — `120`, а создание приглашений дополнительно ограничено до `10` запросов в минуту с одного IP. При превышении сервер отвечает `429 Too Many Requests` и возвращает `Retry-After`.
...
- API создания, публичного чтения и защищённого чтения автора через `/api/v1/`.
...
- Общий и отдельный для создания rate limiting с ответом `429 Too Many Requests`.
...
Следующий небольшой этап — сохранять окончательный ответ получателя (`да` или `нет`) в базе и показывать его на странице автора. После этого можно реализовать совместный выбор даты, времени и места, а затем итоговое подтверждение плана.
